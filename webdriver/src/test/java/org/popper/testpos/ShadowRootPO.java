...
@Page(name = "ShadowDom")
public interface ShadowRootPO {
    @PageAccessor(uri="/pages/shadowRoot.html")
    void open();
    
    @Locator(cssSelector = "nonShadow", getShadowRoot = false)
    ILabel nonShadow();
    
    @Locator(cssSelector = "#shadowRoot", getShadowRoot = true)
    ShadowDomPO shadow();

    public static interface ShadowDomPO {
        @Locator(cssSelector = "button")
        IButton button();
    }
}
