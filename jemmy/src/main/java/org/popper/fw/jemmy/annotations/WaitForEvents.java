...
@Documented
@ImplementedBy(WaitForEventsImpl.class)
@RunBefore({Locator.class})
public @interface WaitForEvents {

}

class WaitForEventsImpl implements IAnnotationProcessor<WaitForEvents, Void> {
    @Override
    public Void processAnnotation(WaitForEvents locatorAnnotation, LocatorContextInformation info, Void lastResult)
            throws ReEvalutateException {
        ((JemmyContext) info.getPopperContext()).waitForEvents();
        return null;
    }
}